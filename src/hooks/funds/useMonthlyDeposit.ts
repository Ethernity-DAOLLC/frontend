...
interface UseMonthlyDepositProps {
...
export function useMonthlyDeposit({
  fundAddress,
  monthlyAmount,
  onSuccess,
  onError,
}: UseMonthlyDepositProps): UseMonthlyDepositReturn {
  const { address: userAddress } = useAccount();
  const usdcAddress = useUSDCAddress();
  const [step, setStep] = useState<MonthlyDepositStep>('idle');
  const [error, setError] = useState<string | null>(null);
  const feeAmount = (monthlyAmount * 3n) / 100n;
  const netToFund = (monthlyAmount * 97n) / 100n;
  ...
  const approve = useCallback(async () => {
    ...
    if (!hasEnoughUSDC) {
      const err = new Error(
        ...
        `Required: ${(Number(monthlyAmount) / 1e6).toFixed(2)} USDC\n` +
  ...
  }, [userAddress, fundAddress, needsApproval, step, writeDeposit, onError]);
  ...
  useEffect(() => {
    if (isDepositSuccess && depositHash && step === 'confirming') {
      console.log('✅ Monthly deposit successful!', depositHash);
      setStep('success');
